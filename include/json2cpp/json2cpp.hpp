...
public:
  ...
  constexpr const basic_json &operator[](std::basic_string_view<CharType> key) const { return find(key)->second; }
  ...
  constexpr const basic_json &at(std::basic_string_view<CharType> key) const
  {
    ...
    if (const auto it = find(key); it != object_data().end()) { return it->second; }
  ...
  {
    if (is_object()) return find(key) != object_data().end();

    if (is_array()) {
      const auto arr = array_data();
      auto it = std::ranges::find_if(arr, [key](const auto &element) { return element.is_string() && element == key; });
      return it != arr.end();
    }
    return false;
  }
